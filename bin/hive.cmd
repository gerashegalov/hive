...
if defined HIVE_MAIN_CLIENT_DEBUG_OPTS (
	set HADOOP_CLIENT_OPTS=%HADOOP_CLIENT_OPTS% %HIVE_MAIN_CLIENT_DEBUG_OPTS% 
...
set HADOOP_HOME=%BASEMAPR%\hadoop\hadoop-0.20.2
...
if not exist %HADOOP% (
	set HADOOP=%HADOOP_HOME%\bin\hadoop.bat
)
if not exist %HADOOP% (
...
:AddServices
	...
	set /a SERVICE_COUNT = %SERVICE_COUNT% + 1
	set VAR%SERVICE_COUNT%=metatool
	
    set /a SERVICE_COUNT = %SERVICE_COUNT% + 1
    set VAR%SERVICE_COUNT%=beeline
goto :EOF
